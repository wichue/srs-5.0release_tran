...
private:
    // Reload  the vhost section of config.重新加载配置的vhost部分。
...
private:
    // Reload the rtc_server section of config. 重载rtc_server部分的配置
    virtual srs_error_t reload_rtc_server(SrsConfDirective* old_root);
    // Reload  the transcode section of vhost of config.重新加载配置的vhost的转码部分。
    virtual srs_error_t reload_transcode(SrsConfDirective* new_vhost, SrsConfDirective* old_vhost);
    // Reload  the ingest section of vhost of config.重新加载配置的虚拟主机的摄取部分。
...
public:
    ...
    // 将http_api部分转储到json中，以获取原始的api信息。
